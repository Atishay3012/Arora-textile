document.addEventListener('DOMContentLoaded', function() {
    ...
    selectElements.forEach(selectElement => {
        ...
        .then(data => {
            ...
            total_price.innerHTML = `Rs ${data.total}`;
            var cart_total = document.getElementById('cart_total');
            cart_total.innerHTML = `Rs ${data.cart_total_before_discount}`;
            document.getElementById('discount').innerHTML = `Rs ${data.discount}`;

            document.getElementById('cart_total_discount').innerHTML = `Rs ${data.cart_total}`;
        })
        .catch((error) => {
...
function removeCoupon() {
    ...
    .then(data => {
        ...
        //remove the discount 
        document.getElementById('discount').innerHTML = `Rs 0.00`;
        // remove coupon div
        let couponDiv = document.querySelector('#coupon-div');
        if (couponDiv) {
...
function removeFromCart(event, product_id, product_size, product_color ) {
    ...
    cart_total.innerHTML = `Rs ${data.cart_total}`;
    
    setTimeout(function() {
        row.parentNode.removeChild(row);
    }, 1000);
        var cart_total = document.getElementById('cart_total').innerHTML = `Rs ${data.cart_total_before_discount}`;
        var cart_total = document.getElementById('discount').innerHTML = `Rs ${data.discount}`;
